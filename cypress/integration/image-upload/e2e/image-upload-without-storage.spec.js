describe('Page: Upload page, test: Success image uploader without Local Storage', () => {
    it('open', () => {
        cy.visit('/');
    });
    ...
    it('onClick button save without Saved image to Local Storage', () => {
        cy.get('[data-test-id="button-save-image"]').contains('Save it!').click().should(() => {
            expect(localStorage.getItem('image')).to.be.contain('')
        });
    });
